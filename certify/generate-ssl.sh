...
openssl req -x509 -config certificate-authority-options.conf -new -nodes -key "tmp/${name}CA.key" -sha256 -days 1825 -out "../${name}CA.pem"
...
openssl req -new -config certificate-authority-options.conf -key "../${name}.key" -out "tmp/${name}.csr"

# Generate SSL Certificate
openssl x509 -req -in "tmp/${name}.csr" -CA "../${name}CA.pem" -CAkey "tmp/${name}CA.key" -CAcreateserial -out "../${name}.crt" -days 1825 -sha256 -extfile options.conf
...
echo "All done! Check the .. directory for your certs."
